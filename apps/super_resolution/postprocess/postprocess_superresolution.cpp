...
class PostprocSR : public cnstream::Postproc {
 public:
  int Execute(const std::vector<float*>& net_outputs, const std::shared_ptr<edk::ModelLoader>& model,
    ...
    std::string save_name1 = output_dir_ + std::string("/output/img_bgr_resize") +
                             std::to_string(package->frame.frame_id) + std::string(".png");
    ...
    std::string save_name2 =
        output_dir_ + std::string("/output/imgSR_dst") + std::to_string(package->frame.frame_id) + std::string(".png");
