...
int FFSws::LockOpt() {
  ...
  swsctx_ = sws_getContext(src_w_, src_h_, src_pix_fmt_, dst_w_, dst_h_,
...
int FFSws::Convert(const uint8_t* const srcSlice[], const int srcStride[],
                 int srcSliceY, int srcSliceH, uint8_t* const dst[],
                 const int dstStride[]) {
  int ret = -1;
  LOCK();
  CHECKNOTSTOP();
  ret = sws_scale(swsctx_, srcSlice, srcStride, srcSliceY, srcSliceH,
                  dst, dstStride);
  return ret;
}
