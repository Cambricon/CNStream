...
class FFSws {
 public:
  ...
  int LockOpt();
  int UnlockOpt();
  int Convert(const uint8_t* const srcSlice[], const int srcStride[], int srcSliceY, int srcSliceH,
              uint8_t* const dst[], const int dstStride[]);
  int Convert(const uint8_t* src_buffer, const size_t src_buffer_size, uint8_t* dst_buffer,
